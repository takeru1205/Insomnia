...
class DQN:
    def __init__(self, env, net=None, buffer=None, gamma=0.95, criterion=None, optimizer=None):
        self.env = env
        self.policy_net = simple_net.SimpleNet(env.observation_space.shape[0],
                                               env.action_space.n) if net is None else net
        self.target_net = deepcopy(self.policy_net)
        self.buffer = replay_buffer.ReplayBuffer([env.observation_space.shape[0]],
                                                 env.action_space.n) if buffer is None else buffer
        self.gamma = gamma
